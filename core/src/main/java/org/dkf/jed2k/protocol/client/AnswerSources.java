package org.dkf.jed2k.protocol.client;

import org.dkf.jed2k.Utils;
import org.dkf.jed2k.exception.JED2KException;
import org.dkf.jed2k.hash.MD4;
import org.dkf.jed2k.protocol.Dispatchable;
import org.dkf.jed2k.protocol.Dispatcher;
import org.dkf.jed2k.protocol.Hash;
import org.dkf.jed2k.protocol.Endpoint;
import org.dkf.jed2k.protocol.Serializable;

import java.nio.ByteBuffer;
import java.nio.ByteOrder;
import java.util.ArrayList;
import java.util.List;

/**
 * OP_ANSWERSOURCES (0x82): client answers with sources for a requested file.
 * Format: <HASH 16><count 2>(<ID 4><PORT 2>)[count]
 * Sources with LowID (<16777216) are filtered out by eMule.
 */
public class AnswerSources implements Serializable, Dispatchable {
    public Hash fileHash = new Hash();
    public List<Endpoint> sources = new ArrayList<>();

    @Override
    public ByteBuffer get(ByteBuffer src) throws JED2KException {
        src.order(ByteOrder.BIG_ENDIAN);
        fileHash.get(src);
        int count = src.getShort() & 0xFFFF;
        sources.clear();
        for (int i = 0; i < count && src.remaining() >= 6; i++) {
            int ip = src.getInt();
            int port = src.getShort() & 0xFFFF;
            if (!Utils.isLowId(ip)) {
                sources.add(new Endpoint(ip, port));
            }
        }
        return src;
    }

    @Override
    public ByteBuffer put(ByteBuffer dst) throws JED2KException {
        dst.order(ByteOrder.BIG_ENDIAN);
        fileHash.put(dst);
        dst.putShort((short) sources.size());
        for (Endpoint ep : sources) {
            dst.putInt(ep.getIP());
            dst.putShort((short) ep.getPort());
        }
        return dst;
    }

    @Override
    public int bytesCount() {
        return MD4.HASH_SIZE + 2 + sources.size() * 6;
    }

    @Override
    public void dispatch(Dispatcher dispatcher) throws JED2KException {
        dispatcher.onAnswerSources(this);
    }
}
